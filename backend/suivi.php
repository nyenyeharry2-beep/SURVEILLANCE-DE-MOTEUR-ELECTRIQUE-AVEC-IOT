    ...
    if ($matricule === '') {
        ...
        try {
            ...
            if (!$student) {
                ...
                $feesStmt = $pdo->prepare('SELECT label, montant_du, montant_paye, statut, mois FROM student_fees WHERE student_id = ? ORDER BY statut, mois');
    ...
    <div class="<?= $tab === 'accueil' ? '' : 'hidden' ?>">
        <div class="card">
            ...
            <?php if ($searchResult): ?>
                ...
                <?php else: ?>
                    <h2 style="margin-top:14px;">Détail des frais</h2>
                    <?php foreach ($searchResult['fees'] as $f): ?>
                        <div class="fee">
                            <span><?= htmlspecialchars($f['label']) ?><?= $f['mois'] ? ' (mois '.$f['mois'].')' : '' ?></span>
