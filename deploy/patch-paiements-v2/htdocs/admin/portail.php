            ...
            if (!move_uploaded_file($_FILES['pdf']['tmp_name'], $destPath)) {
                ...
                try {
                    ...
                    if ($result['processed'] > 0) {
                        if ($type === 'paiements' && isset($result['inserted'])) {
                            $uploadMessage = sprintf(
                                'Import paiements OK : %d ligne(s) — %d nouvelle(s), %d mise(s) à jour, %d non reconnue(s), %d erreur(s)',
                                $result['processed'],
                                $result['inserted'],
                                $result['updated'],
                                $result['unmatched'],
                        ...
                        $uploadError = sprintf(
                            'Aucune donnée importée. %d matricule(s) CSLSG détecté(s) dans le PDF (%d caractères lus). Importez d\'abord les inscriptions, puis les paiements (connexe, minerval, bus…).',
                            $matriculesDetectes,
                            strlen($text)
                        );
                    }
    ...
    <div id="tab-imports" class="<?= $tab === 'imports' ? '' : 'hidden' ?>">
        ...
        <div class="card">
            ...
            <form method="post" enctype="multipart/form-data" action="portail.php?tab=imports">
                <label>Type</label>
                <select name="type">
                    <option value="auto">Auto (détecte inscriptions / paiements)</option>
                    <option value="inscriptions">Inscriptions (liste élèves + matricules)</option>
                    <option value="paiements">Paiements (connexe, minerval, bus, équipements…)</option>
                </select>
                <p class="hint" style="font-size:0.85rem;color:#666;margin:8px 0;">Les rapports de paiement sans matricule sont rapprochés par <strong>nom de l'élève</strong>. Réimporter un PDF met à jour les paiements existants (retards inclus).</p>
