...
const state = {
  ...
  firebaseReady: false,
  thresholds: { vibMax: 100, rpmMax: 5000 },
  sampleWindow: [],
  history: [],
...
function applyMeasurement(raw = {}, source = "mysql") {
  ...
  const commandedOn = Boolean(raw.motorOn ?? state.commande.etatCommande);
  ...
  const etat = raw.emergency
    ? "défaut"
    : raw.alert || raw.reason === "vibration_haute" || raw.reason === "rpm_haute"
      ? "alerte"
      : deriveMotorState({ rpm, rmsMmS }, extras);
        ...
        <section class="panel motor-control" data-section="overview commande">
          <div>
            ...
            <p class="muted">
              ...
              (<code>motor/command</code>). L’ESP32 lit la commande
              et actionne le relais UNO.
            </p>
          </div>
...
function updateDashboard() {
  ...
  const firebaseOk = state.mysqlStatus === "connected";
      ...
      state.mysqlStatus === "connected"
        ...
        : state.mysqlStatus === "error"
          ...
          : state.mysqlStatus === "disconnected"
            ? "Hors ligne"
            : "Firebase…";
    fb.className = "status-pill " + (firebaseOk ? "ok" : "bad");
  }
...
async function sendMotorCommand(on) {
  ...
  try {
    ...
    toast(on ? "Commande ALLUMER envoyée à Firebase" : "Commande ÉTEINDRE envoyée à Firebase");
  } catch (error) {
    state.mysqlError = error.message;
    toast(state.demoMode ? "Commande appliquée en démonstration" : state.mysqlError);
    updateDashboard();
  }
}
...
function startFirebaseListeners() {
  ...
  try {
    ...
    subscribeConnected((online) => {
      state.mysqlStatus = online ? "connected" : "disconnected";
      state.mysqlError = online ? "" : "Connexion Firebase perdue.";
      if (state.dashboardMounted) updateDashboard();
    });

    subscribeThresholds((thresholds) => {
      state.thresholds = thresholds;
    });

    subscribeCommand((cmd) => {
      if (!state.demoMode) applyCommandeData(cmd);
      if (state.dashboardMounted) updateDashboard();
    });

    subscribeLive((row) => {
      if (state.demoMode) return;
      const mapped = mapFirebaseLive(row, state.thresholds);
      if (mapped) {
        applyMeasurement(mapped, "firebase");
      } else {
        state.dataSource = "none";
      }
      if (state.dashboardMounted) updateDashboard();
...
async function boot() {
  ...
  if (user) onSignedIn(user);
    ...
    state.error = "Impossible de démarrer le tableau de bord.";
    render();
  }
}
