...
(function () {
  ...
  function isFirebaseConfigured(config) {
    return config
      && config.databaseURL
      && !config.databaseURL.includes("VOTRE_PROJET")
      && config.apiKey
      && !config.apiKey.includes("VOTRE_");
  }

  if (!isFirebaseConfigured(cfg)) {
    console.warn("Configurez Web/firebase-config.js avec vos clés Firebase.");
    el.connBadge.textContent = "Firebase non configuré";
    el.connBadge.className = "badge badge-off";
    el.alertBanner.classList.remove("hidden");
    el.alertBannerText.textContent =
      "Firebase non configuré : ouvrez firebase-config.js et collez vos clés (databaseURL obligatoire). " +
      "Avec Lumen ESP32, renseignez aussi FIREBASE_DB_URL et FIREBASE_AUTH dans config.php.";
    setInterval(() => {
