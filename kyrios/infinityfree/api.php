<?php
// KYRIOS API — compatible InfinityFree (sans .htaccess)
...
// Route via ?route=auth/login (compatible InfinityFree)
$uri = $_GET['route'] ?? 'health';
$uri = trim($uri, '/');
$method = $_SERVER['REQUEST_METHOD'];

try {
    if ($uri === 'health' && $method === 'GET') {
        jsonResponse(['status' => 'ok', 'app' => 'KYRIOS', 'host' => 'InfinityFree', 'url' => 'kyrios.infinityfree.me']);
    }
    ...
    jsonError('Route introuvable: ' . $uri, 404);

} catch (PDOException $e) {
    jsonError('Erreur base de donnees: ' . $e->getMessage(), 500);
} catch (Exception $e) {
