...
void MotorController::begin() {
  ...
  disarm();
  Serial.println("[MOTOR] Controleur pret");
}
...
void MotorController::setOutputs(float throttle01, float rollCmd, float pitchCmd, float yawCmd) {
  ...
  // Mixage X — adapter les signes selon le sens de rotation de vos moteurs
...
uint16_t MotorController::mixMotor(float throttle01, float roll, float pitch, float yaw,
                                   float signRoll, float signPitch, float signYaw) const {
